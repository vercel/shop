...
The storefront also publishes `/llms.txt`, a concise index for AI agents. It links to up to 50 collections, the sitemap, crawl guidance, and the storefront's UCP profile rather than duplicating the full catalog. The [sitemap](/docs/anatomy/sitemap) remains the complete URL inventory.

Markdown explains what a product or collection is; it does not list or sell inventory. Catalog search, live per-variant availability, cart, and checkout for agents come from Shopify's Universal Commerce Protocol (UCP) endpoints, which the storefront forwards at `/.well-known/ucp` and `/api/ucp/mcp`. Markdown responses point agents there instead of repeating product listings or per-variant stock.
...
| Surface                          | Outcome                                                                        |
| -------------------------------- | ------------------------------------------------------------------------------ |
| Markdown responses               | Describes products and collections and points AI clients to UCP                |
...
- **Collections** — title, description, image, and links to the shopper page and UCP catalog search. Collection contents change often, so products are not listed.
- **Search** — the query and links to the shopper results page and UCP catalog search.
