...
- **Rate limit** - a per-IP fixed window (20 requests / 60s) returns 429 with `Retry-After`. The default primitive in `lib/agent/rate-limit.ts` is in-memory and best-effort - swap for Vercel KV / Upstash / Runtime Cache in production
