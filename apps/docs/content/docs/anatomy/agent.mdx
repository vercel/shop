...
1. Set `agent.isEnabled` to `true` in `lib/config/index.ts`. The chat button appears in a fixed corner control by default; set `agent.position` to `"inline"` to show it with the header icons instead.
