...
- **[Vercel BotID](https://vercel.com/docs/botid)** — wrap `next.config.ts` with `withBotId()`, call `initBotId({ protect: [{ path: "/api/chat", method: "POST" }] })` from `instrumentation-client.ts`, and call `checkBotId()` at the top of the POST handler. Returns 403 when a bot is detected and works locally without a Vercel deployment.
...
| `lib/agent/server.ts` | ToolLoopAgent creation, dynamic system prompt, and AsyncLocalStorage context (chatId, user, cart, page) |
