...
Set `agent.isEnabled` to `true` in `lib/config/index.ts`, then deploy with access to the Vercel AI Gateway. The included assistant uses OpenAI GPT-5.6 Luna by default.
