...
**Bot protection is on by default.** Enabling the assistant also enables [Vercel BotID](https://vercel.com/docs/botid), an invisible CAPTCHA that verifies a request came from a real browser session. The chat endpoint is anonymous and every message costs model tokens, so it is a natural target for automated abuse. Verification runs before the request body is parsed and before a cart is created, and a request that fails is rejected with a `403`. BotID runs at the free `basic` level unless configured otherwise; see [Shop Configuration](/docs/reference/shop-config) to change the level or turn it off.
...
- **Rate limiting and spend caps** — BotID rejects automated traffic, but it doesn't bound what a real browser can spend. Per-IP rate limiting backed by shared infrastructure such as Vercel Runtime Cache, plus a model output/token cap, bound worst-case gateway cost. Run rate-limit checks alongside the existing bot check, before the request body is parsed.
