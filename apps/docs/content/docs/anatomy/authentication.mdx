...
| Module | Purpose |
|--------|---------|
| `lib/auth/auth.ts` | Core better-auth configuration with Shopify OIDC via the `genericOAuth` plugin. Exports `isAuthEnabled`. Server-only. |
...
The `isAuthEnabled` flag (from `lib/auth/auth.ts`) reads `NEXT_PUBLIC_AUTH_ENABLED`. When `false`:
...
- `isAuthEnabled` must read a `NEXT_PUBLIC_` variable — server-only env vars cause hydration mismatches with cache components
