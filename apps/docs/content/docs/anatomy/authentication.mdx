...
Authentication is enabled by default on the enterprise branch. Complete the Shopify setup below, or set `auth.isEnabled` to `false` in `lib/config.ts` to run a guest-only storefront.
