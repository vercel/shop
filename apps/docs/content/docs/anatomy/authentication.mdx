...
Keep both secrets server-only and identical across every instance of a deployment. Rotating the session secret signs out all customers. Use the numeric shop ID for `NEXT_PUBLIC_SHOP_ID`, not the Headless storefront ID. A build with authentication enabled fails when the shop ID, session secret, client ID, or client secret is missing.
...
9. Set `NEXT_PUBLIC_SHOP_ID` to the shop ID shown in Shopify's default customer accounts URL.
10. Confirm `NEXT_PUBLIC_SHOPIFY_STORE_DOMAIN` identifies the same store.
