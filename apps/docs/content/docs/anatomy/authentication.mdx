...
Add these three environment variables to enable the auth UI:
...
Generate the secret with `openssl rand -base64 32`. The client ID and secret come from Shopify's Customer Account API.
...
Authentication is built on three modules in `lib/auth/`:

| Module | Purpose |
|--------|---------|
| `auth.ts` | Core better-auth configuration with Shopify OIDC via the `genericOAuth` plugin. Exports `isAuthConfigured` for feature-gating. |
| `server.ts` | Server-side session helpers: `getCustomerSession()`, `getSession()`, `requireCustomerSession()`, `requireSession()`. Uses React `cache()` for per-request memoization. |
| `client.ts` | Client-side hooks and actions: `useSession()`, `signIn()`, `signOut()`. |
...
The `isAuthConfigured` flag (exported from `lib/auth/auth.ts`) checks whether all three required env vars are present. When `false`:

- The account icon does not appear in the nav
- `/account/login` and `/account/*` return 404
- No auth-related code runs at request time

This means auth infrastructure has zero runtime cost when unconfigured.
