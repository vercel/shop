...
| Module                                | Purpose                                                                                                                                          |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `lib/shopify/fetch.ts`                | `customerAccountFetch()` — POSTs to `https://{domain}/customer/api/{version}/graphql` with the customer's access token as the raw `Authorization` header (no `Bearer` prefix). |
