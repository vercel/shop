...
If sign-in redirects fail, compare the deployed origin, callback URI, logout URI, store domain, and client ID character for character. Also confirm that a confidential client has its matching client secret; failed callbacks return to the storefront instead of restarting sign-in.
