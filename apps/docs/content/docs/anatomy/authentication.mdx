...
Keep both secrets server-only and identical across every instance of a deployment. Rotating the session secret signs out all customers. Use the numeric shop ID for `NEXT_PUBLIC_SHOPIFY_SHOP_ID`, not the Headless storefront ID. Every build requires the shop ID; enabling authentication also requires the session secret, client ID, and client secret.
...
9. Set `NEXT_PUBLIC_SHOPIFY_SHOP_ID` to the shop ID shown in Shopify's default customer accounts URL.
