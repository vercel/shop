...
Authentication is disabled by default. The storefront remains guest-only until you enable `auth.isEnabled` in `lib/config/index.ts` and complete the Shopify setup below.
...
<Callout type="note" title="Local HTTPS with Portless">
  For local HTTPS, install [Portless](https://portless.sh) with `pnpm add -g portless`, then run
  `pnpm dev:portless`. Use HTTPS when testing OAuth callbacks or secure cookies.
</Callout>
