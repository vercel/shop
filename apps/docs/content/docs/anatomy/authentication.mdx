...
Shopify provides the client ID. If it provides a confidential client, also set `SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_SECRET` to the client secret. Leave that variable unset for a public client. Generate the session secret yourself:
...
5. Use a public Customer Account API client when available. If Shopify provides a confidential client, copy both its client ID and client secret.
...
If sign-in redirects fail, compare the deployed origin, callback URI, logout URI, store domain, and client ID character for character. Also confirm that a confidential client has its matching client secret; failed callbacks return to the account page instead of restarting sign-in.
