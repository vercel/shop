...
**Mutations post directly to Hydrogen's cart endpoint.** The client helpers in `lib/cart/client.ts` send `fetch()` requests to `POST /api/cart`. Add-to-cart, quantity updates, and removal dispatch Hydrogen's standard `shopify:cart:lines-update` event with the request promise, so the cart store owns optimistic state, pending work, reconciliation, and rollback. Discount apply/remove await the endpoint and hand the resolved cart to the overlay through the local `shop:cart:discount-resolved` event, so a new code never flashes an "invalid" pill while the server catches up.
