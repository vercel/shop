...
The optional shopping assistant uses the same cart. The storefront refreshes it after the assistant's response settles. Stopping a response does not roll back a Shopify write that has already started. Checkout remains unavailable during the response and cart reconciliation; an unconfirmed write can temporarily block checkout for up to two minutes. Inspect the cart before requesting another change. Restoring an old conversation does not replay its cart changes.
