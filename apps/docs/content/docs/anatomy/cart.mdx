...
**Mutations go through Hydrogen's cart forms.** `useCartForm()` returns `formProps()` and `register()` helpers that encode the cart action contract; submitting a registered form routes through the store's `handleFormSubmit` to `POST /api/cart`. Discount forms use Hydrogen's `discount-apply` and `discount-remove` intents, read their loading state from `pending.discountCodes`, and surface scoped errors from `errors.discountCodes`. Newly submitted codes remain visually pending until Shopify confirms whether they apply, so the optimistic placeholder does not flash as invalid. Form submissions without JavaScript still work: the endpoint answers a native form POST with a `303` redirect and the cart cookie set.
