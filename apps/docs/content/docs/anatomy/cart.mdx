...
Cart operations in `lib/commerce/` use API mutations for adding, updating, and removing cart lines. The cart ID is stored in a `cartId` HTTP-only cookie with a 7-day expiry. Every mutation calls `invalidateCartCache()` to ensure subsequent reads reflect the latest state.

The transform layer in `lib/commerce/` converts the provider's API response shape into a domain `Cart` type used throughout the application. This isolates the rest of the codebase from the provider's API structure.
...
| `lib/commerce/` | Cart operations and API calls |
| `lib/commerce/` | Provider response to domain type transform |
