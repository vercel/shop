...
Updates are optimistic: quantities and badges change immediately, then reconcile with Shopify. If Shopify rejects a change, the cart returns to its confirmed state and shows the relevant error. Totals show an updating state while Shopify confirms the cost. Checkout remains unavailable until pending cart changes finish. Line-specific errors appear alongside the affected item.
