...
- **Server actions** (`lib/cart/action.ts`, `"use server"`) — handle the checkout-URL flows: `buyNowAction()` (checkout URL for a new item) and `prepareCheckoutAction()` (checkout URL for an existing cart). Line and discount mutations go through `/api/cart`. Locale and currency are set at cart creation through the `I18nConfig` (country/language) that `seedCartData()` passes to `createShopifyRequestContext()`; Hydrogen applies it via `@inContext`.
