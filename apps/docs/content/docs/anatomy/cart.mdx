...
**Mutations reconcile through Hydrogen's cart store.** The template posts line and discount mutations directly to `/api/cart`, then dispatches Hydrogen's standard cart events with the request promise so the store owns optimistic state, pending work, reconciliation, and scoped errors. Discount forms read loading state from `pending.discountCodes` and errors from `errors.discountCodes`; newly submitted codes remain visually pending until Shopify confirms whether they apply, so the optimistic placeholder does not flash as invalid.
