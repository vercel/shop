...
The cart displays confirmed discounts and sale pricing, using the same monospace prices as product pages and cards. Shopify bundles remain grouped so shoppers can understand which products belong together. Quantity and remove controls follow Shopify's rules for each line, so some bundle lines may not be editable individually.
