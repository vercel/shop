---
title: Cart
description: The cart system - state management, optimistic updates, server actions, and commerce provider integration.
type: guide
---

<CartBrowser />

> **Note:** The cart ID is stored in a `cartId` cookie with a 7-day expiry. Sessions longer than 7 days start a fresh cart.

The cart system spans the entire storefront, not just the cart page. A React Context provider wraps the app in `layout.tsx`, giving every component access to cart state. Mutations flow through server actions that call the commerce provider's API, while optimistic updates keep the UI responsive during network round-trips.
...
- `buyNowAction()` - adds an item and returns the checkout URL
...
Cart operations in `lib/commerce/operations/cart.ts` use GraphQL mutations: `cartLinesAdd`, `cartLinesUpdate`, and `cartLinesRemove`. The cart ID is stored in a `cartId` HTTP-only cookie with a 7-day expiry. Every mutation calls `invalidateCartCache()` to ensure subsequent reads reflect the latest state.

The transform layer in `lib/commerce/transforms/cart.ts` converts the provider's GraphQL response shape into a domain `Cart` type used throughout the application. This isolates the rest of the codebase from the provider's API structure.
...
| `lib/commerce/operations/cart.ts` | Cart GraphQL operations |
| `lib/commerce/transforms/cart.ts` | Provider response to domain type transform |
