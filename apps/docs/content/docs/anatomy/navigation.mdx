...
The navigation system is split into a sticky top navbar for desktop and a fixed bottom bar for mobile. By default, navigation links are hardcoded with a multi-column Shop menu and placeholder footer groups. Use [`/vercel-shop:enable-shopify-menus`](/docs/skills/enable-shopify-menus) to fetch menus from Shopify.
...
A horizontal link bar with hardcoded defaults: `Shop` and `About`. `Shop` links to `/search` and includes a five-column placeholder mega menu with product links at nested levels. The component already accepts a Shopify-shaped `MenuItem[]` and renders up to three levels via an inline hover panel; the `enable-shopify-menus` skill swaps the data source over to a Shopify Navigation menu. Hidden on mobile (`md:flex`).
...
By default the footer renders placeholder link columns, a copyright notice, and optional social links. The `enable-shopify-menus` skill replaces the fallback footer columns with Shopify-powered headings and links.
...
| `components/footer/index.tsx` | Footer columns, copyright, and social links slot |
