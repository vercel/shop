...
Unknown handles return a not-found page by default. You can opt into [Shopify redirects](/docs/anatomy/proxy#shopify-redirects-on-missing-pages) for missing pages.

There is no combined `/blogs` page, so navigation should link to a specific blog such as `/blogs/news`.

While content loads, a placeholder shows the shape of the page, article, or blog listing.
