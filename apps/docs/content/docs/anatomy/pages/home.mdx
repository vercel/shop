...
The home page is a server component that renders a hero banner, a featured products carousel, and an informational section. Product data is fetched server-side from the commerce provider.
...
Products are fetched server-side with `getProducts({ limit: 10, locale })`, which queries the commerce API and caches the result with `cacheLife("max")` and product-level cache tags for granular revalidation.
...
| `lib/commerce/` | Product fetching with caching |
