...
The enterprise demo’s “Picked for you” section uses the most recently visited collection, remembered in a cookie for 30 days. An allowed campaign collection takes priority; without either, the section shows jackets.

Returning home updates only this section when the remembered collection changes, without refreshing the page or clearing cached navigation. Hovering a collection link does not count as a visit. The initial grid renders on the server; subsequent updates show the same loading skeleton while the new products load.
