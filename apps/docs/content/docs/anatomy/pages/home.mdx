...
By default, when no `backgroundImage` or `backgroundVideo` is provided, the hero renders on a solid `bg-foreground` background with no image. To add a background image, pass a `backgroundImage` prop — either a static import (`import myHero from "@/public/my-hero.jpg"`), which enables automatic blur placeholders and prevents layout shift, or a remote URL object (`{ url: "https://...", alt: "..." }`).
