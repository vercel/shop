...
`partialFallbacks` enables Partial Pre-Rendering (PPR), where the static shell of the page is served instantly and dynamic sections stream in. `unstable_instant` provides `unstable_samples` with representative parameters so the framework can pre-render the static shell at build time. `unstable_prefetch = "force-runtime"` tells Next.js to prefetch dynamic data at request time rather than statically.
