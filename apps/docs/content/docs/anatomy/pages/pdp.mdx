...
- **Buy with Shop** - creates a checkout and redirects to it
...
Below the product details, a `Recommendations` component fetches related products from the provider's recommendation API. It renders inside a `Suspense` boundary with a skeleton grid fallback.
...
| `components/product/pdp/buy-section-client.tsx` | Quantity selector, Buy with Shop, add to cart |
| `components/product/pdp/mobile-buy-buttons.tsx` | Simplified mobile buy buttons (Buy with Shop + add to cart) |
