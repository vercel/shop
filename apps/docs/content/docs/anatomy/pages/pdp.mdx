...
The PDP runs under Next.js 16 Cache Components. Product data is fetched through `"use cache: remote"` operations with `cacheLife("max")` and a `product-{handle}` tag, so the page renders from the shared Runtime Cache on every request and is only rebuilt when a Shopify webhook invalidates the matching tag — ISR-style freshness with no per-request fetch (see [Shopify Integration](/docs/shopify) for the webhook setup).
