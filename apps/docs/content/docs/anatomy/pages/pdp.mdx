...
Like bundle relationships, the section renders **eagerly into the prerendered shell** rather than streaming. Its parent in `product-detail-section.tsx` owns the `pdp.upsells.enabled` gate and does not render the component when disabled, so the cached (`use cache: remote`) `getComplementaryProducts()` operation does not run. The operation fetches only the `COMPLEMENTARY` intent and uses the `complementary-{handle}` invalidation tag. When enabled, the result is baked into the static PPR/ISR document — there's no skeleton fallback and no layout shift on cached hits (a `Suspense` boundary would instead make it a streamed hole). It shows up to `pdp.upsells.limit` products as thumbnail + title + price rows and renders nothing when none are configured.
