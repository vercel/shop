...
- **Buy Now** — creates a checkout and redirects to it
...
Below the product details, a `Recommendations` component fetches related products from the commerce provider's recommendation API. It renders inside a `Suspense` boundary with a skeleton grid fallback.
