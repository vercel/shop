...
While the fast options promise resolves, the picker streams behind a Suspense fallback that renders with the **default selection** already highlighted (`defaultSelectedOptions(product)`), not an empty selection. For the common no-params load the resolved highlight matches the fallback, so nothing moves; a deep link to a non-default variant shifts the highlight by one pill rather than popping a selection in from nothing.
