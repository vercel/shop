...
Swatch availability comes from Shopify's `encodedVariantAvailability` field, decoded in `lib/shopify/encoded-variants.ts` into the set of in-stock values per option. Text option pills render in three tiers — the selected value gets a `foreground` (near-black) border and text, other in-stock values a `muted-foreground` (dark gray) border and text that darkens toward the selected treatment on hover, and unavailable values a lighter `border`-toned, struck-through, `cursor-not-allowed` state. Unavailable options render as inert `<span>` elements instead of links.
