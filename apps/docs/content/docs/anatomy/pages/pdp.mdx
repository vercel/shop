...
The picker highlight only appears once that fast options promise resolves — while it streams, the Suspense fallback renders with **no** active selection rather than eagerly highlighting the default value. Eagerly seeding the default would make every option switch briefly flash the default pill (e.g. tapping M while on XS momentarily re-highlights S) before the chosen value resolves; deferring the highlight to hydration avoids that bounce. The softened border-only selected state (below) keeps the highlight appearing on hydration from reading as a pop.

Swatch availability comes from Shopify's `encodedVariantAvailability` field, decoded in `lib/shopify/encoded-variants.ts` into the set of in-stock values per option. Text option pills render in three tiers — the selected value gets a solid `foreground` (near-black) border and text; other in-stock values a solid, light `border`-toned pill with `muted-foreground` text that darkens toward the selected treatment on hover; and unavailable values a **dashed** `border` with lighter, struck-through text and `cursor-not-allowed`. All three tiers share one border width so the pill geometry never shifts between states. When the selection resolves, only the selected pill animates: its `starting:border-border starting:text-muted-foreground` (CSS `@starting-style`) makes it ease up from the neutral border/text into the active near-black on the same `transition-all` the hover uses, so the active state comes to life instead of popping in — the other pills render at their final state with no entrance animation. Unavailable options render as inert `<span>` elements instead of links.
