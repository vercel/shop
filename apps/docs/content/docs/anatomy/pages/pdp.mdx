...
Two prefetch opt-ins make option clicks instant, and both are required: the route exports `prefetch = "allow-runtime"` (permits runtime prefetching), and the option and color picker links set `prefetch={true}` — without it a `Link` only prefetches the per-route App Shell, so the runtime prefetch never fires and every option click pays a server roundtrip.
