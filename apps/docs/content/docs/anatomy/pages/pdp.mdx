...
The route passes `searchParams` into a selection promise consumed only by selection-dependent Suspense boundaries. When a numeric variant is present, that promise performs two operations:
