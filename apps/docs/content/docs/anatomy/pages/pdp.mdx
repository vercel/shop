...
When `pdp.complementaryProducts.enabled` is `true` in `shop.config.ts`, `complementary-products.tsx` renders the merchant-curated "Pairs Well With" set below the bundle relationships — Shopify's [complementary products](/docs/shopify/pdp#complementary-products) from `productRecommendations(intent: COMPLEMENTARY)`. These are distinct from the algorithmic related products below the fold (`intent: RELATED`): complementary products are deliberately chosen add-ons, so they sit beside the buy section like bundle relationships rather than in the footer grid.

Like bundle relationships, the section renders **eagerly into the prerendered shell** rather than streaming. Its parent in `product-detail-section.tsx` owns the `pdp.complementaryProducts.enabled` gate and does not render the component when disabled, so the cached (`use cache: remote`) `getComplementaryProducts()` operation does not run. The operation fetches only the `COMPLEMENTARY` intent and uses the existing `recommendations-{handle}` invalidation tag. When enabled, the result is baked into the static PPR/ISR document — there's no skeleton fallback and no layout shift on cached hits (a `Suspense` boundary would instead make it a streamed hole). It shows up to `pdp.complementaryProducts.limit` products as thumbnail + title + price rows and renders nothing when none are configured.
...
| File                                                   | Purpose                                                                                                              |
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `shop.config.ts`                                       | PDP bundle, complementary-product, related-product, and specification policy                                         |
