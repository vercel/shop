...
- **Gift cards** — collects recipient details and keeps gifts for different recipients separate in the cart. If adding a gift card fails, recipient details and the delivery schedule stay in the form for retry. The form clears only after the add succeeds.
- **Buy with Shop** — starts Shopify checkout.
- **Bundles** — shows included items and other bundles containing the product.
- **Complementary products** — presents merchant-curated add-ons with the same monospace, primary-color prices as product cards.
