...
The PDP uses a rendering strategy combining `unstable_instant`, `unstable_prefetch`, and `experimental.partialFallbacks`:
...
`partialFallbacks` enables Partial Pre-Rendering (PPR), where the static shell of the page is served instantly and dynamic sections stream in. `unstable_instant` provides `samples` with representative parameters so the framework can pre-render the static shell at build time. `unstable_prefetch = "force-runtime"` tells Next.js to prefetch dynamic data at request time rather than statically.
...
Related products display as a horizontally scrollable carousel of product cards using the same `ProductsCarousel` component as the homepage. On mobile, cards are ~7/12 viewport width and scroll full-bleed. On desktop, the carousel shows 3-4+ columns with chevron navigation.
