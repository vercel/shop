...
Product pages show the title, price, media, description, and available options. Option choices update the page URL, so shoppers can share a selected color, size, or other variant. Selecting an option replaces the current history entry rather than adding a new one for each choice.
