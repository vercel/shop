...
Pagination is cursor-based using the provider's cursor rather than page numbers. A First/Next button pair navigates between pages, with loading indicators and disabled states at boundaries. The cursor resets automatically when filters or sort change.
...
Products within a collection are fetched via `getCollectionProducts()`, which accepts the provider's filter type for server-side filtering by variant options, price, vendor, type, tags, and metafields.

> **Note:** Filters are driven entirely by the provider's filter type. Adding custom filter logic (e.g., filtering by a metafield) requires modifying the query in `lib/commerce/`.

## Search page

The `/search` route accepts a `q` query parameter and combines it with the same filter and sort controls. `getProducts()` builds a search query from the text input and active filters, mapping UI sort keys to the provider's sort keys.
...
| `lib/commerce/` | Product search with caching and filter mapping |
| `lib/commerce/` | Collection metadata fetching |
