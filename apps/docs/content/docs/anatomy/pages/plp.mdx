...
Each product renders as a card with the featured image (with hover slideshow on desktop), title, price, compare-at price, availability status, and a quick-add button for in-stock items.
...
The `/search` route accepts a `q` query parameter and combines it with the same filter, sort, and infinite scroll controls. The search page displays a custom header with the query and result count, and shows a "no results" message with i18n support when the query returns empty.
...
| `components/product-card/product-card.tsx` | Product card with image, hover slideshow, price, quick-add |
