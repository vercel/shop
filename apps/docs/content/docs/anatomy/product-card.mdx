---
title: Product Card
description: The product card component - compound structure, variants, image and fallback behavior, and a recipe for adding a hover image carousel.
type: guide
---

The product card is the shared tile used wherever products appear in a list: the home featured grid, collection (PLP) and search grids, the related-products slider on the PDP, and the AI agent's results. It is a compound component, so each piece can be composed independently while sharing the same styling hooks.

## Composition

The high-level `ProductCard` (`components/product-card/product-card.tsx`) is a server component that wraps the product in a `Link` to `/products/[handle]` (appending `?variant=` when a default variant id is known) and assembles the compound pieces from `components/product-card/components.tsx`:

| Part                        | Element   | Purpose                                                                          |
| --------------------------- | --------- | -------------------------------------------------------------------------------- |
| `ProductCard`               | `article` | Root wrapper. Carries the `default` / `featured` variant.                        |
| `ProductCardBadge`          | `div`     | Optional corner badge (used by the `featured` variant).                          |
| `ProductCardImageContainer` | `div`     | Holds the image and content; applies the `featured` gradient.                    |
| `ProductCardImage`          | `div`     | Renders the featured image, the no-image fallback, and the out-of-stock overlay. |
| `ProductCardContent`        | `div`     | Wraps the title and price.                                                       |
| `ProductCardTitle`          | `h3`      | Product title, clamped to two lines.                                             |
| `ProductCardPrice`          | `div`     | Price, optional compare-at price, and a discount badge.                          |

Each part exposes a `data-slot` attribute (`product-card`, `product-card-image`, etc.) as a stable styling hook.
...
| Surface                 | Component                                            |
| ----------------------- | ---------------------------------------------------- |
| Home featured grid      | `components/product/products-grid.tsx`               |
| PDP related products    | `components/product/products-slider.tsx`             |
| Collection (PLP) grid   | `components/collections/results-grid.tsx`            |
| Infinite scroll batches | `components/collections/infinite-product-grid.tsx`   |
| Search results          | `components/search/results.tsx`                      |
| AI agent results        | `components/agent/registry.tsx` (`AgentProductCard`) |
