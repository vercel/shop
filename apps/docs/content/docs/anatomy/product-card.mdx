...
The high-level `ProductCard` (`components/product-card/product-card.tsx`) is a server component that wraps the product in a `Link` to `/products/[handle]` (appending `?variant=` when a default variant id is known) and composes the smaller parts from `components/product-card/components.tsx` — the image container, image, content, title, price, and an optional badge. Each part exposes a `data-slot` attribute (`product-card`, `product-card-image`, etc.) as a stable styling hook.
