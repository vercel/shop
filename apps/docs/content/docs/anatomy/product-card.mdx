...
When a shopper applies one color filter, collection and search cards use the image assigned to that color's first selectable variant and link to its selected options. On hover, the card shows the first gallery image not assigned to a color variant, such as a model or lifestyle photo. Assign an image to each color variant in Shopify so filtered cards can distinguish merchandise images from shared gallery images. Cards fall back to the product image when the matching variant has no image.
