...
The proxy also forwards `/.well-known/ucp` to Shopify’s Universal Commerce Protocol (UCP) business profile. The optional assistant publishes a separate agent profile at `/agent/ucp-profile.json`. Eve's `/eve/v1/` session routes bypass Shopify routing.
