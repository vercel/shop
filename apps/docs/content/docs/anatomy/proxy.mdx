...
Shopify's redirect fallback is disabled by default. Set `notFound.shopifyRedirects.isEnabled` to `true` in `lib/config/index.ts` to enable Hydrogen's handler on the storefront's not-found page.
