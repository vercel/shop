...
The proxy forwards `/.well-known/ucp` to Shopify’s Universal Commerce Protocol (UCP) business profile. It also forwards `POST /api/ucp/mcp` to Shopify. Callers supply their own authorization and UCP agent profile; storefront customer sessions do not authorize these requests. UCP MCP responses are not publicly cacheable.
