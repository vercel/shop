...
When `ucp.isEnabled` is enabled, the proxy also serves Shopify’s [UCP business profile](/docs/anatomy/ucp) at `/.well-known/ucp`. This endpoint returns `404` while disabled.
