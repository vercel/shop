...
The handler never invalidates the broad `products` tag, and fires the broad `collections` tag only on `collections/create`. Those tags wrap aggregate reads, so firing one on a routine edit would needlessly bust the whole set. The handler relies on fine-grained tags instead, which is sufficient because the reads in `lib/shopify/operations/` stamp each aggregate cache entry with the granular tag of every item it contains — `tagProducts()` adds a `product-{numericId}` tag per product (on list/search/collection/recommendation reads), and `getCollections`/`getCollectionsListing` add a `collection-{handle}` tag per collection:
...
- **Collection topics** fire the `collection-{handle}` tag. It busts that collection's PLP reads — `getCollection` (metadata) and `getCollectionProducts` (the product list, including membership and sort changes) — and, because the all-collections reads stamp every collection they return with that same tag, it also busts the listing whenever an existing collection is edited or deleted. A brand-new collection has no tag on the already-cached listing, so `collections/create` **also** fires the broad `collections` tag to surface it. That tag is confined to the only reads where the full set matters — the all-collections listing (`getCollections`, `getCollectionsListing`) and the collections sitemap — so create refreshes those without touching any other collection's PLP. Product cards inside a collection list are independently covered by their own `product-{numericId}` tags via the product webhook. Navigation menus aren't busted here; menu titles and links are edited independently in Shopify Navigation, and there is no menu webhook topic.
