...
Product notifications refresh the affected product wherever it appears, including recommendations and sitemap content. They also invalidate initial collection grids and their facets, including All products: an edit can change which products belong in a result or their order, even when the edited product was not previously shown. Product creation and deletion also refresh product sitemap membership.
...
Malformed product payloads still invalidate initial collection grids but cannot identify a specific product to refresh. Malformed collection payloads cannot identify a collection to refresh. Valid Shopify deliveries include the expected identifiers.
