...
Webhooks are disabled by default. Without `SHOPIFY_WEBHOOK_SECRET`, the endpoint returns `404`. Edits appear when cached content refreshes or the site is redeployed; cache eviction can make them appear sooner.
...
You can register only the topics your store uses. Changes without a registered topic have no event-driven refresh.
...
The included handler does not refresh inventory-level changes. Stock shown for the default variant can lag until the product cache refreshes; selecting a variant reads its current availability from Shopify.

This handler does not refresh Online Store pages, blogs, articles, or store policies. Those edits appear when cached content refreshes unless you arrange a separate refresh. See [Content pages](/docs/anatomy/pages/content) for publishing guidance.
