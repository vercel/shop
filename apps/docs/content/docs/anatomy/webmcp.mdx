...
WebMCP and Shopify analytics are separate choices. Enabling WebMCP loads Shopify's browser scripts, including consent, but does not enable Shopify page-view reporting.
...
4. Set `shopify.webmcp.isEnabled` to `true` in `lib/config/index.ts`, then load the site.
