...
For upgrade work, use the [`update-shop` skill](/docs/skills/update-shop). The template publishes a changelog of change-level entries, so agents can reason about individual updates instead of guessing from a single scaffold version. New scaffolds record bootstrap metadata so agents can compare changelog entries against when a project was created.
