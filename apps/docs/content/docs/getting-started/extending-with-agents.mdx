...
To initialize a storefront from an agent, use the `init-vercel-shop` skill and provide an explicit target directory. The skill delegates filesystem changes to `create-vercel-shop`, verifies the generated project and bootstrap metadata, and reports any plugin installation that needs a retry.
