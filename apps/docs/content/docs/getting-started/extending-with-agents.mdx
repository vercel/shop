...
Reusable shapes for common storefront customizations, each naming the component, the behavior, and the constraint the agent needs.

- **Adding UI to a page** — `Add [component] to [page]. It should [behavior]. Pull data from [source]. Show it [condition].`
...
- **Modifying existing behavior** — `Change [component] so that [new behavior] instead of [current behavior]. Keep [constraint].`
...
- **Integrating a third-party service** — `Integrate [service] for [purpose]. Use their [SDK/API]. Add it to [page/component].`
...
- **Styling changes** — `Update [component] to [visual description]. Match the existing design system.`
...
Challenges ranked by complexity. Each one is a single agent prompt - describe what you want and let the agent handle the implementation.
