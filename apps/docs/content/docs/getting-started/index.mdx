...
`NEXT_PUBLIC_AUTH_ENABLED=1` turns on the auth UI (account icon, login page, account pages). The remaining three are server-side secrets. Without these variables, the storefront works as a guest-only experience. See [Environment Variables](/docs/reference/env-vars) for details.
