...
- **`.claude/schemas/`** - local Shopify GraphQL schema snapshots. The agent uses these to write type-safe queries without guessing field names.
- **`.agents/skills/`** - step-by-step playbooks for common tasks like enabling Shopify Markets, adding customer auth, or writing Shopify GraphQL against the local schema snapshots.
