...
| `NEXT_PUBLIC_SHOP_ID`               | Required when Shopify analytics, consent, WebMCP, or customer authentication is enabled. Use the numeric shop ID.   |
...
The numeric shop ID is distinct from the Headless storefront ID. Shopify's default customer accounts URL has the form `https://shopify.com/123456789/account`, where `123456789` is the shop ID. Builds fail if a feature needs `NEXT_PUBLIC_SHOP_ID` and the value is missing or not numeric.
