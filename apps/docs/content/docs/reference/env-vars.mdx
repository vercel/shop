...
These three values identify your Shopify store and connect the storefront.
...
| `NEXT_PUBLIC_SHOPIFY_SHOP_ID`                 | Numeric Shopify shop ID, distinct from the Headless storefront ID.                                    |

Shopify's default customer accounts URL has the form `https://shopify.com/123456789/account`, where `123456789` is the shop ID. Builds fail if `NEXT_PUBLIC_SHOPIFY_SHOP_ID` is missing.
