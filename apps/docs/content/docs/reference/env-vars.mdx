...
These variables enable built-in customer authentication with [better-auth](https://www.better-auth.com/) and Shopify Customer Account API OIDC. Set `NEXT_PUBLIC_AUTH_ENABLED=1` to turn on the auth UI, then provide the three server-side secrets.

| Variable | Description |
|----------|-------------|
| `NEXT_PUBLIC_AUTH_ENABLED` | Set to `1` to enable the auth UI (account icon, login page, account pages). Must be a public variable for hydration compatibility with cache components. |
