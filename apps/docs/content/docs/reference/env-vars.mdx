...
| `NEXT_PUBLIC_ENABLE_BOTID`             | Set to `"1"` or `"0"` to control `botid.enabled`, which protects the chat endpoint with [Vercel BotID](https://vercel.com/docs/botid). Defaults to whatever the agent flag resolves to. Set `"0"` when self-hosting off Vercel. |
| `NEXT_PUBLIC_ENABLE_SHOPIFY_ANALYTICS` | Set to `"1"` or `"0"` to control `analytics.shopify.enabled`, which sends storefront page views to Shopify analytics. Disabled by default; set `"1"` to turn it on. Uses the Storefront API credentials above.                  |
| `NEXT_PUBLIC_SHOPIFY_CONSENT_MODE`     | Set to `"default-banner"`, `"custom-banner"`, or `"no-banner"` to control `analytics.shopify.consentMode`. Defaults to `no-banner` in development and `default-banner` in production.                                           |
...
| `NEXT_PUBLIC_BOTID_CHECK_LEVEL` | Set to `"deepAnalysis"` to use BotID's machine-learning detection, which is billed per check on Pro plans. Defaults to `"basic"`, free on all plans. |
