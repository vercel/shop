...
| `NEXT_PUBLIC_ENABLE_SHOPIFY_ANALYTICS` | Set to `"1"` or `"0"` to control `analytics.shopify.enabled`, which sends storefront page views to Shopify analytics. Enabled by default; uses the Storefront API credentials above.                                            |
