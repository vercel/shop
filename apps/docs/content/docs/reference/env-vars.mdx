...
| `NEXT_PUBLIC_ENABLE_AGENT` | Set to `"1"` to show the AI shopping assistant (eve). Its model routes through the Vercel AI Gateway — uses the project OIDC token on Vercel, or set `AI_GATEWAY_API_KEY` elsewhere. |
