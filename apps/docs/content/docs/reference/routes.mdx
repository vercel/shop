...
PDP option links use option-name query parameters so Shopify can resolve high-variant products and Combined Listings without loading every variant. Legacy Shopify `variant` URLs remain accepted:

| URL                                              | Purpose                                                            |
| ------------------------------------------------ | ------------------------------------------------------------------ |
| `/products/:handle?Color=Black&Size=Medium`      | Canonical PDP option-selection URL                                 |
| `/products/:handle?variant=:variantId`           | Legacy-compatible URL; resolves the ID to selected options first   |
