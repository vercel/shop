...
The PDP capability flags are disabled by default so a basic storefront does not query Shopify features it does not use:
...
These flags describe optional Shopify capabilities with a data-fetching cost. Display counts and component composition remain implementation details rather than configuration.
