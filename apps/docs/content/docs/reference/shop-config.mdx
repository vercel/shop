...
Each recommendation component owns its gate and returns before fetching when disabled. Complementary and related products use separate cached Shopify operations and invalidation tags, so enabling one surface does not request data for the other.
