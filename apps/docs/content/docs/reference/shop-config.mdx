...
`shopify.enabled` sends storefront page views to Shopify analytics through Hydrogen's analytics bus, so they appear in Shopify admin alongside the rest of the store's reporting. It is disabled by default and needs no credentials beyond the Storefront API variables the storefront already requires. Set `NEXT_PUBLIC_ENABLE_SHOPIFY_ANALYTICS="1"` to turn it on for a deployment.
