...
Do not put secrets, access tokens, store-specific product data, Shopify query fields, display counts, component-level feature switches, or AI runtime wiring in `shop.config.ts`. Keep credentials in environment variables, merchandising data in Shopify, and implementation policy beside the code that owns it.
