...
**Customer accounts** require a Shopify client ID, Shopify client secret, and a separate app-generated session secret in addition to the shop ID required for every build. Follow [Authentication](/docs/anatomy/authentication) to configure those values and register callback URLs before enabling `auth.isEnabled`.
...
Set the required `NEXT_PUBLIC_SHOPIFY_SHOP_ID` to the numeric shop ID, not the Headless storefront ID.
