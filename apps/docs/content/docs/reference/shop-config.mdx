...
| ------------------------------------- | ---------------- | ----------------------------------------------------------------------------------------------------------- |
| `agent.isEnabled`                     | `true`           | Shows the shopping assistant and mounts Eve's session routes. Requires the assistant environment variables. |
...
| `botid.isEnabled`                     | `false`          | Protects assistant setup and Eve session requests with [Vercel BotID](https://vercel.com/docs/botid).       |
| `botid.checkLevel`                    | `basic`          | Selects `basic` or `deepAnalysis` detection.                                                                |
| `localization.country`                | `"US"`           | Sets the country sent to Shopify for commerce context.                                                      |
| `localization.language`               | `"EN"`           | Requests catalog content in this language from Shopify.                                                     |
| `localization.locale`                 | `"en-US"`        | Sets display formatting; does not translate labels or select currency.                                      |
...
With `agent.isEnabled` enabled by default, configure `AGENT_SESSION_SECRET`, `AGENT_REDIS_URL`, `AGENT_REDIS_TOKEN`, and `AGENT_STOREFRONT_URL`. Follow the [assistant setup guide](/docs/anatomy/agent); customer login is not required.

Before enabling `auth.isEnabled`, set `CUSTOMER_ACCOUNT_SESSION_SECRET`, `SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_ID`, and `SHOPIFY_CUSTOMER_ACCOUNT_API_CLIENT_SECRET` as described in [Environment Variables](/docs/reference/env-vars). Development startup and production builds fail if any of these variables are missing.
