...
Shopify consent loads when Shopify analytics, standalone consent, or WebMCP is enabled. If all three are off, the storefront still loads Shopify cart actions, but skips the analytics and consent scripts and their shop-data lookup. These modes apply whenever Shopify consent loads.
