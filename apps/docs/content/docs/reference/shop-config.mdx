...
Customer accounts, Shop Agent, analytics, bot protection, WebMCP, and Shopify redirects on missing pages are **disabled by default**. Enable only the features you intend to use and complete their setup before deploying.
...
| `analytics.shopify.isEnabled`         | `false`              | Enables Shopify analytics and its consent integration.                                                                         |
| `analytics.shopify.consent.isEnabled` | `false`              | Enables Shopify consent without enabling Shopify analytics.                                                                    |
| `analytics.shopify.consent.mode`      | `default-banner`     | Chooses how shoppers give consent; see below.                                                                                  |
...
| `redirects.shopifyNotFound.isEnabled` | `false`              | Enables [Shopify redirect lookup](/docs/anatomy/proxy#shopify-redirects-on-missing-pages) when a storefront page is not found. |
| `search.isEnabled`                    | `true`               | Shows the header search button and predictive search dialog.                                                                   |
| `shopify.webmcp.isEnabled`            | `false`              | Registers Shopify catalog and cart tools with WebMCP in supported browsers.                                                    |
...
**Customer accounts** require a numeric Shopify shop ID, a Shopify client ID, a Shopify client secret, and a separate app-generated session secret. Follow [Authentication](/docs/anatomy/authentication) to configure those values and register callback URLs before enabling `auth.isEnabled`.

**Shopify analytics, consent, and WebMCP** also require `NEXT_PUBLIC_SHOP_ID`. Set it to the numeric shop ID before enabling any of these features. It is different from the Headless storefront ID.
...
Shopify consent loads when Shopify analytics, standalone consent, or WebMCP is enabled. If all three are off, the storefront still loads Shopify cart actions but skips the analytics and consent scripts. These modes apply whenever Shopify consent loads.
