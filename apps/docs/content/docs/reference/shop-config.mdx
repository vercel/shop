...
| `ucp.isEnabled`                       | `false`          | Publishes Shopify’s UCP business profile at `GET /.well-known/ucp`.                             |
...
UCP is disabled by default. When `ucp.isEnabled` is `true`, `GET /.well-known/ucp` serves the business profile generated by the configured Shopify store. See [Universal Commerce Protocol](/docs/anatomy/ucp) for setup and limits.
