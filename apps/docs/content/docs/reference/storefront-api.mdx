...
Most stable read operations use `"use cache: remote"` with `cacheLife("max")` and one or more cache tags. The remote directive stores entries in Vercel's shared Runtime Cache, so they persist across serverless instances and requests. Search, filters, sort, and cursor reads also use the remote cache with their function arguments as part of the key; the high-cardinality product selection read is uncached.
