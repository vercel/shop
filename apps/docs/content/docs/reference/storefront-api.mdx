...
The template does not query an exhaustive variants connection. The PDP starts with `getProduct()` in its prerendered shell and derives the default selection from that cached product when no valid variant ID is present. For `/products/:handle?variant=:variantId`, a selection promise uses `getProductVariantRouteSelection()` to validate the numeric ID and retrieve Shopify's selected options, then passes those options to `getProductSelection()`. Only selection-dependent Suspense regions consume that promise. The selection operation uses:
...
Shopify Liquid links already use the public `?variant=:variantId` contract. The PDP performs the uncached `node(id:)` validation directly and corrects the handle for Combined Listings before calling the shared selection operation. There is no rewrite or internal variant route. Product metadata canonicalizes variants to the base product path.
