...
Hydrogen's `gql()` infers result types from the Storefront API schema bundled with the SDK. Inference is not validation: a field that does not exist, or exists only in the newer `SHOPIFY_API_VERSION` the template queries, types as `unknown` and fails only where it is consumed. Validate new or changed documents with Shopify AI Toolkit before relying on them, and exercise the operation against your store once. Invalid documents surface at runtime through `assertStorefrontOk()`.
