...
The template does not query an exhaustive variants connection. The PDP starts with `getProduct()` in its prerendered shell and derives the default selection from that cached product when no valid variant ID is present. For `/products/:handle?variant=:variantId`, a selection promise first checks the cached product's representative variants. The default variant needs no additional query, known representative variants already contain their selected options, and only unknown IDs use `getProductVariantRouteSelection()` for validation. The selected options then pass to `getProductSelection()`. Only selection-dependent Suspense regions consume that promise. The selection operation uses:
...
Shopify Liquid links already use the public `?variant=:variantId` contract. When the cached representative set cannot identify the ID, the PDP performs the uncached `node(id:)` validation directly. A cross-handle Combined Listing link redirects to the owning child product before calling the shared selection operation. There is no rewrite or internal variant route. Product metadata canonicalizes variants to the base product path.
