...
1. **`"use cache: remote"`** - stores the entry in Vercel's shared Runtime Cache, durable across serverless instances and requests
...
Read operations use `"use cache: remote"` with `cacheLife("max")` and one or more cache tags. The remote directive stores entries in Vercel's shared Runtime Cache, so they persist across serverless instances and between requests rather than the per-instance in-memory store that plain `"use cache"` falls back to at runtime. Combined with webhook-driven `revalidateTag()` invalidation, reads stay cached until the underlying Shopify data changes. Tags follow a hierarchy:
