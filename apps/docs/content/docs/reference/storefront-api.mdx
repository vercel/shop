...
Run the Storefront schema check from your project root after editing operations. It uses [`@shopify/api-codegen-preset`](https://www.npmjs.com/package/@shopify/api-codegen-preset) to validate Storefront `#graphql` documents under `lib/shopify/`:

```bash
npm run codegen
```

Codegen reads `SHOPIFY_API_VERSION` from the command's environment and checks operations against Shopify's live schema. If you override the API version, pass the same value when running codegen. The development command runs this check before starting Next.js but continues if it fails; the production build stops on failure. Generated validation files are gitignored.
