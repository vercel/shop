...
The template does not query an exhaustive variants connection. The PDP uses the static-shell `getProduct()` read plus a short-TTL `getProductSelection()` request for option-dependent regions. The selection operation uses:
...
`getProductSelection()` uses `"use cache: remote"` with `cacheLife("minutes")`, keyed by canonically sorted options. The short TTL keeps the high-cardinality combination keyspace transient while reusing recently resolved selections. It is also used by tools that need to resolve an exact option combination, such as the shopping agent. Product cards link to the bare product path; PDP option links use option-name query parameters rather than numeric variant IDs.
