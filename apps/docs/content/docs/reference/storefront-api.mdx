...
The operation name is appended as a query parameter (`?operation=getProductByHandle`) so calls are easy to identify in network logs. Cache Components key cached operation results from function arguments; the operation chooses plain or remote caching based on its role in the consuming route.
