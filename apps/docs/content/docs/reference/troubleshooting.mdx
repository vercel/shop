...
Single-locale storefronts inherit the shop's default Shopify currency for pricing and filter chrome. Multi-locale support still requires Shopify Markets to be enabled. Run [`/vercel-shop:enable-shopify-markets`](/docs/skills/enable-shopify-markets) to add multi-locale and multi-currency routing.
