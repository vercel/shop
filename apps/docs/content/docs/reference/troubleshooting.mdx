...
**Check:** Validate the operation against the live Storefront API schema for the configured `SHOPIFY_API_VERSION` using Shopify AI Toolkit. A field that exists only in a newer API version types as `unknown` under Hydrogen's inference, so typecheck does not catch it.
