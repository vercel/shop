...
Existing Liquid links such as `?variant=123` should permanently redirect to an option-name URL. If they do not, verify `getProductVariantRouteSelection()` can resolve the numeric ID and that the Storefront token can access the variant.
