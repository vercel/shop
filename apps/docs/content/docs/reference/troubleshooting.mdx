...
PDP option links use `/products/:handle?variant=:variantId`. If an option link does not select the expected variant, check that the transformed option value has a numeric variant ID and the selected variant's owning product handle.

The PDP first checks the cached product's representative variants. It calls `getProductVariantRouteSelection()` with `cache: "no-store"` only when that set does not contain the requested ID, then calls `getProductSelection()` with Shopify's selected options. Confirm the final selection returns the same numeric variant ID. For external or stale Combined Listing links, the route should permanently redirect when Shopify reports a different owning product handle.

Existing Liquid links such as `?variant=123` should render the selected variant directly. If they do not, verify `app/products/[handle]/page.tsx` still reads and validates the numeric query value.

The route should remain Partial Prerendered in `next build` output. If it becomes fully dynamic, check that `page.tsx` does not await `searchParams` before rendering and that only selection-dependent Suspense regions consume the selection promise.

Option-name query parameters are intentionally ignored by the default template. Use the `enable-partial-product-selection` skill only when the storefront needs shareable incomplete choices.
