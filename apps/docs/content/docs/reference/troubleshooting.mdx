...
The PDP first calls `getProductVariantRouteSelection()` with `cache: "no-store"`, then calls `getProductSelection()` with Shopify's selected options. Confirm the lookup returns the same numeric variant ID after selection. For Combined Listings, the route should permanently redirect when Shopify reports a different owning product handle.
