...
By default, the template uses hardcoded navigation and footer links from `lib/config.ts`, including a placeholder Shop mega menu. Use [`/vercel-shop:enable-shopify-menus`](/docs/skills/enable-shopify-menus) to replace them with dynamic menus fetched from Shopify.
