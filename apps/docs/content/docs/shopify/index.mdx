...
Most read operations use `"use cache: remote"` and `cacheLife("max")`. The PDP base-product read uses plain `"use cache"` so it can be baked into the static shell, while option selections use the remote cache with `cacheLife("minutes")`. Without webhooks, long-lived entries won't update until their cache lifetime expires. To get near-instant updates, configure Shopify webhooks to hit the template's invalidation endpoint.
...
| `lib/shopify/fetch.ts`              | Core Storefront API client                                        |
