...
- **Basic fields** — title, `descriptionHtml`, variants, featured image, and up to 10 media items (images or videos). Product gallery images use optional blur placeholders from the Storefront-public `custom.image_placeholders` JSON metafield when present. Define it as a product metafield with JSON type and Storefront access, using MediaImage GIDs as keys and `{ blurDataURL, width, height }` as values. Products must belong to at least one collection to appear on collection listing pages.
