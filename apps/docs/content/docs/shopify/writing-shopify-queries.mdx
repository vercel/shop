...
If you're using a coding agent, run `/write-shopify-graphql` in Claude Code or ask the agent to use the `write-shopify-graphql` skill first. The skill follows the same workflow below and keeps the agent anchored to the local schema snapshots and template guardrails.
