...
- **`"use cache: remote"`** stores the entry in Vercel's shared Runtime Cache, so it persists across serverless instances and between requests. Plain `"use cache"` is per-instance in-memory at runtime — it only survives for params prerendered at build, so most reads would refetch on every cold instance
