...
If you're using a coding agent, run `/shopify-graphql-reference` in Claude Code or ask the agent to use the `shopify-graphql-reference` skill first. The skill is the reference source for Shopify GraphQL work and keeps the agent anchored to the bundled schema snapshots and template guardrails.
