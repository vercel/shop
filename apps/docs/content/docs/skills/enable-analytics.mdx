...
The storefront sends page views to Shopify analytics through Hydrogen's analytics bus. It is disabled by default and requires no credentials beyond the Storefront API variables the storefront already needs. To turn it on for a deployment, set `NEXT_PUBLIC_ENABLE_SHOPIFY_ANALYTICS="1"`.
