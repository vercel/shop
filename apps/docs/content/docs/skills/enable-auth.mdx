...
{/* BEGIN SKILL CONTENT: enable-auth */}

# Enable Authentication

Add customer authentication using [better-auth](https://www.better-auth.com/) with your commerce provider's OIDC. This enables customer login, profile management, order history, and address book.

## Prerequisites

- Commerce provider with **Customer Account API** enabled (check your provider's admin settings)
...
| `OIDC_CLIENT_ID`                | Customer Account API client ID                                   |
| `OIDC_CLIENT_SECRET`            | Customer Account API client secret                               |
| `BETTER_AUTH_BASE_URL`           | App base URL (e.g. `http://localhost:3000` for dev)              |
| `COMMERCE_STORE_DOMAIN`         | Already set — used for OIDC discovery                            |
...
Core better-auth configuration with OIDC provider:

```ts
import { betterAuth } from "better-auth/minimal";
import { genericOAuth } from "better-auth/plugins";

const COMMERCE_STORE_DOMAIN = process.env.COMMERCE_STORE_DOMAIN;

if (!COMMERCE_STORE_DOMAIN) {
  console.warn("[better-auth] COMMERCE_STORE_DOMAIN not set - auth will not work");
}

const OIDC_SCOPES = ["openid", "email", "customer-account-api:full"];
      ...
      config: [
        {
          ...
          discoveryUrl: COMMERCE_STORE_DOMAIN
            ? `https://${COMMERCE_STORE_DOMAIN}/.well-known/openid-configuration`
            : undefined,
          scopes: OIDC_SCOPES,
          pkce: true,
          accessType: "offline",
...
export function signIn(callbackURL = "/account"): void {
  authClient.signIn.oauth2({ providerId: "commerce", callbackURL });
}
...
1. Go to **your provider's admin** and navigate to customer account settings
2. Enable **Customer Account API**
3. Create a **Customer Account API client** (under "API clients")
4. Set the redirect URI to `{YOUR_DOMAIN}/api/auth/callback/commerce`
5. Copy the client ID and client secret to your environment variables
6. Ensure the store domain matches `COMMERCE_STORE_DOMAIN`
...
- The Customer Account API uses a separate GraphQL endpoint from the Storefront API — always reference the customer account GraphQL schema
...
Create `lib/commerce/types/customer.ts` with domain types for Customer, Address, Order, Fulfillment, and related types.

Create `lib/commerce/operations/customer.ts` with:
...
Reference the customer account GraphQL schema for field names. All operations use the Customer Account API (separate from Storefront API) with OAuth Bearer tokens.
