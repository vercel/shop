...
```
Error: Route "/[locale]/..." accessed [...] which is not defined in the `unstable_samples` of `unstable_instant`.
```
...
### E. `unstable_instant` `unstable_samples` need `locale` in `params`

Any route that exports `unstable_instant` (currently: products `[handle]`, collections `[handle]`, search) needs `locale` added to every sample, or validation fails:

```
Error: Route "/[locale]/products/[handle]" accessed root param "locale"
       which is not defined in the `unstable_samples` of `unstable_instant`.
```
...
### F. `unstable_instant` `unstable_samples` need `headers` declarations if any layout-level server component reads `headers()`
...
Error: Route "..." accessed header "x-vercel-ip-postal-code" which is not
       defined in the `unstable_samples` of `unstable_instant`. Add it to the
...
Walk every route file that exports `unstable_instant` and add `locale` to each `unstable_samples` entry's `params`:
