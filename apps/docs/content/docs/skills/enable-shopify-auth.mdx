...
As of April 2026, **Part 1** (core auth plumbing) and the nav account icon are built into the template by default. Run this skill only if you need the full account pages (profile, orders, addresses) from Part 2.
...
{/* BEGIN SKILL CONTENT: enable-shopify-auth */}
...
{/* END SKILL CONTENT: enable-shopify-auth */}
