...
> **Note:** Product selection stays in query parameters: option-name params are canonical and legacy `?variant=` links remain supported. The built-in content negotiation rewrite in `next.config.ts` handles markdown negotiation automatically — no proxy.ts changes needed.
...
4. **Variants**: Confirm option-name query params and legacy `?variant=` links survive locale-prefixed navigation
