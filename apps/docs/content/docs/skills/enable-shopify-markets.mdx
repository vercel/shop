...
> **Note:** Normalized product selection stays in option-name query parameters. Shopify-standard `?variant=` migration links permanently redirect to that format. The built-in content negotiation rewrite in `next.config.ts` handles markdown negotiation automatically — no proxy.ts changes needed.
