...
4. **Always reference `.claude/schemas/` when writing GraphQL**. Never guess field names.

## Overview

This is a Next.js 16 commerce storefront template. It uses the App Router, React 19, Server Components, Tailwind CSS 4, and pnpm. Commerce operations are abstracted behind a `CommerceProvider` interface in `lib/commerce/`.
...
- `app/` for routes
- `lib/commerce/` for commerce provider abstraction, operations, and types
...
```text
Request → Page → commerce.<operation>() → Provider API → Transform → Domain type → Component
```
...
- Commerce operations: `recipes/commerce/operations.md`
...
- Components import domain types from `@/lib/types`, not provider-specific response types.
...
Reference local schema snapshots in `.claude/schemas/` when writing or reviewing GraphQL for a specific provider.
