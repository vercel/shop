...
- Use grouped component-local class maps with the type-only `VariantProps<typeof styles>` helper from `@/lib/variants` and `cn()` for multi-variant component APIs; keep defaults in parameter destructuring and let `null` omit a variant.
