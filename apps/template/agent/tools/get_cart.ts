...
import { fetchCart } from "@/lib/shopify/operations/cart.fetch";
...
Use this when the user asks "what's in my cart?", "how much is my order?", or when you need to look up line item IDs for updating/removing items.`,
  ...
  async execute(_input, ctx) {
    ...
    if (!cartId) {
      return { success: true, empty: true, message: "Cart is empty", items: [] };
    }
    ...
    try {
      ...
      return {
        ...
        items: cart.lines.map((line) => ({
          ...
          options: line.merchandise.selectedOptions.map((o) => `${o.name}: ${o.value}`).join(", "),
