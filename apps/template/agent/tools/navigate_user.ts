...
Returns a URL that the user can click to navigate.`,
  ...
  execute({ destination, identifier }) {
    const urls: Record<string, { url: string; label: string }> = {
      ...
      addresses: { url: "/account/addresses", label: "Your Addresses" },
    };

    if (destination === "product" && identifier) {
      return { success: true, url: `/products/${identifier}`, label: "View Product" };
    }

    if (destination === "collection" && identifier) {
      return { success: true, url: `/collections/${identifier}`, label: "Browse Collection" };
    }
    ...
    if (destination === "checkout") {
      return { success: true, url: "/cart", label: "Go to Cart to Checkout" };
    }
