...
export function AgentPanel({ onOpenChange, open, triggerRef }: AgentPanelProps) {
  ...
  const agent = useEveAgent({
    ...
    async prepareSend(turn) {
      ...
      if (!response.ok) throw new Error(t("errorPrepare"));
      return {
        ...turn,
  ...
  const handleSend = (text: string) => {
    ...
    setAgentCartStatus("pending");
    setControlError(null);
    void agent.send(text).catch(() => setControlError(t("errorSend")));
    setInput("");
  };
  const handleClear = async () => {
    ...
    try {
      ...
      clearChat();
      if (busy || status === "error") setControlError(t("clearedDuringTurn"));
    } catch {
      if (clearAttempt.current === attempt) setControlError(t("errorClearUnconfirmed"));
    } finally {
    ...
    >
      ...
      >
        <div className="flex h-16 shrink-0 items-center justify-between gap-2 px-2.5">
          <SheetTitle className="font-normal text-xl leading-4">{t("name")}</SheetTitle>
        ...
        >
          ...
          >
            {messages.length === 0 ? (
              status === "resuming" ? (
                showRestoring && <p className="text-muted-foreground text-sm">{t("restoring")}</p>
              ) : (
        ...
        {(clearing || (showRestoring && messages.length > 0)) && (
          <p role="status" className="px-2.5 py-2 text-muted-foreground text-xs">
            {clearing ? t("waitingForStop") : t("restoringHint")}
          </p>
        )}
        {hasReachedLimit && (
