...
export function OverlayItem({ item, locale }: OverlayItemProps) {
  ...
  const title = merchandise?.product.title || t("items");
  ...
  const price = (amount: number) =>
    formatMoney({ amount: String(amount), currencyCode }, { locale }).localizedString;
  ...
  return (
    <li className="flex gap-2.5" aria-busy={isPending || undefined} aria-label={title}>
        ...
        <div className="grid gap-2.5">
          <div className="grid gap-1">
            ...
            {item.sellingPlanAllocation ? (
              <p className="text-xs text-muted-foreground">
                {item.sellingPlanAllocation.sellingPlan.name || t("subscription")}
              </p>
            ) : null}
          </div>
          {components.length ? (
                ...
                {components.map((component) => (
                  ...
                  >
                    <span className="truncate">
                      {component.merchandise?.product.title || t("bundleIncludes")}
                    </span>
      ...
      <div className="grid gap-0.5 self-start py-0.5 text-right font-mono text-sm tabular-nums tracking-tight">
        ...
        >
          {isCostPending || !currencyCode ? t("updatingCart") : price(finalUnitPrice)}
        </span>
