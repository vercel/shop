...
async function ProductInfoArea({
  ...
  return (
    <div className={cn("grid gap-10", hasMedia && "lg:sticky lg:top-20 lg:col-span-4")}>
      ...
      {eagerSelection ? (
        <ProductInfoOptions options={eagerSelection.options} t={t} />
      ) : (
        <Suspense fallback={<ProductInfoOptions options={options} t={t} hideImages />}>
          <ResolvedProductInfoOptions selectionPromise={selectionPromise} t={t} />
        </Suspense>
      )}
...
async function ResolvedProductInfoOptions({
  ...
  return <ProductInfoOptions options={options} t={t} />;
}
