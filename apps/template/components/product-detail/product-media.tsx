      ...
      >
        ...
        {mediaItems.map((item, idx) => {
          ...
          return (
            <div
              key={mediaKey(item)}
              className="relative shrink-0 w-full snap-start snap-always overflow-hidden aspect-square bg-accent"
            >
              {item.type === "video" ? (
...
function GridItem({
  ...
  return (
    <div className="relative w-full overflow-hidden aspect-square bg-accent">
      {item.type === "video" ? (
        <MediaVideo
          item={item}
