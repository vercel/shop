...
import type { CartToolsOptions } from "./types";
...
export function createCartTools({ cartId, locale }: CartToolsOptions) {
  const getCart = tool({
    description:
    ...
    execute: async () => {
      ...
      try {
        return cartSummary(await getCartById(cartId, locale));
      } catch (error) {
