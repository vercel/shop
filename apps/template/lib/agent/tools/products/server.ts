...
export function createProductTools(locale: string) {
  const searchProductsTool = tool({
    ...
    execute: async ({ intent, mode, options, query, sortKey }) => {
      ...
      try {
        ...
        if (mode === "semantic") {
          try {
            pool = await semanticProducts(searchQuery, intent, poolSize, locale);
          } catch (error) {
  ...
  return { getProductDetailsTool, getRecommendationsTool, searchProductsTool };
}
