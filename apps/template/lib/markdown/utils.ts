...
export function createTable(headers: string[], rows: string[][]): string {
  ...
  // Validate and normalize rows: ensure all rows have the same number of cells as headers
  const normalizedRows = rows
    .filter((row) => row.length > 0) // Skip completely empty rows
    .map((row) => {
      // Pad rows that are shorter than headers, or trim rows that are longer
