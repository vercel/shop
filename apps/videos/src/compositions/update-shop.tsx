...
const TerminalScene = ({ frame, fps }: { frame: number; fps: number }) => {
  ...
  return (
    <div className="flex h-[400px] flex-col gap-2.5 overflow-hidden">
      ...
      {frame >= auditLine && (
        <div className="flex items-center gap-2 font-mono text-sm text-gray-1000" style={riseIn(frame, auditLine, 5)}>
          <CheckIcon />
          {AUDIT_LINE}
        </div>
      )}
      ...
      {frame >= skipLine && (
        <div className="flex items-start gap-2 font-mono text-sm" style={riseIn(frame, skipLine, 5)}>
          <span className="inline-flex size-4 shrink-0 items-center justify-center text-base leading-none text-gray-600">
            −
          </span>
      ...
      {LOGS.map((log, i) => {
        ...
        return (
          <div className="contents" key={log.text}>
            <div className="flex items-start gap-2 font-mono text-sm" style={riseIn(frame, start)}>
              <span
            ...
            {i === 2 && frame >= diff && (
              <div
                className="flex flex-col gap-1 pl-6 font-mono text-xs leading-[18px]"
        ...
        {showAsk ? (
          ...
          >
            ...
            <div className="flex flex-col gap-0.5 px-2">
              {PLAN.map((change) => (
                <span className="flex items-center gap-2 font-mono text-xs text-gray-800" key={change}>
