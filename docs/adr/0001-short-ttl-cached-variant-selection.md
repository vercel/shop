...
The default storefront exposes `/products/:handle` and Shopify-standard `/products/:handle?variant=:variantId` URLs on one Partial Prerendered PDP route. The cached base product renders in the static shell. A selection promise resolves a numeric ID from the cached representative variants when possible, falling back to an uncached lookup for unknown IDs. It validates the owning product, then calls `getProductSelection()` with Shopify-provided selected options. Only variant-dependent regions consume that promise. Requests without a valid variant ID or requests for the cached default variant derive Shopify's default selection without another query.
...
- Unknown variant IDs use an uncached lookup. The cached representative set bypasses it for the default and commonly linked variants without letting arbitrary numeric IDs create cache entries.
