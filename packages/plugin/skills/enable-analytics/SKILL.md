...
Extend the existing root analytics component. Do not create a sibling `components/analytics.tsx`, which would shadow the directory import. Preserve the Shopify scripts gate: Shopify analytics, standalone consent, and WebMCP each require `ShopifyScriptsTracker` and its shop data. When all three are off, do not fetch shop analytics data:
