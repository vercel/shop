...
Extend the existing root analytics component. Do not create a sibling `components/analytics.tsx`, which would shadow the directory import. Preserve the Shopify scripts gate: Shopify analytics, standalone consent, and WebMCP each require a numeric `NEXT_PUBLIC_SHOP_ID`. Build the ShopifyScripts props from configuration rather than making a shop-data request:
