...
Update `app/pages/[slug]/page.tsx` to use `getMarketingPage`. Add the `/pages/[slug]` URLs from `getAllMarketingPageSlugs` to the static entries in `app/sitemap/[type]/[page]/route.ts`; this preserves the storefront CMS route instead of exposing Shopify's raw metaobject type path.
