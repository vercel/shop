...
Keep the mutation in a server action, validate both inputs, and update the cart's buyer identity directly. The Hydrogen cart stack has no `updateCartBuyerIdentity` operation — read the cart id from the shared `cart` cookie, issue `cartBuyerIdentityUpdate` through `storefront.request`, then call `invalidateCartCache()`:
