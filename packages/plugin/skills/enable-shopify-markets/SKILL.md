...
Preserve any other request handling already composed in `proxy()` and keep the template matcher unless the current checkout adds a route with different requirements. The matcher explicitly excludes framework internals, metadata routes, and common static asset extensions while keeping application routes and `/.well-known/*` in proxy processing.
