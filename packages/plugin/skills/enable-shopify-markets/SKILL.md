...
Preserve any other request handling already composed in `proxy()` and keep the template matcher unless the current checkout adds a route with different requirements. Static assets and file-like metadata routes stay out of proxy processing, while `/.well-known/*` remains included for standards-based application routes.
