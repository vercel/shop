...
### A1. Update `components/nav/quick-links.tsx`
...
### A2. Update `components/nav/index.tsx`
...
### B1. Update `components/footer.tsx`
...
Create a directory `components/nav/megamenu/` with the following files:
...
Import and render the `Megamenu` component in `components/nav/index.tsx`, passing `locale`:
...
The bottom bar (`components/bottom-bar.tsx`) should include a Browse button that toggles the megamenu on mobile. Add:
...
- Components in `components/nav/megamenu/` may import from `@/lib/shopify/types/megamenu` for prop types, but must not call Shopify operations directly — data fetching happens in the server component barrel (`index.tsx`).
