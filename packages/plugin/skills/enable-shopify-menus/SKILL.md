...
async function Copyright() {
  await connection();
  const copyright = await t("footer.copyright", { year: String(new Date().getFullYear()) });
...
It needs three translated strings from the `nav` namespace, passed in as props from the server parent (no `useTranslations` — the template resolves translations server-side and prop-drills the result):

- `categoriesLabel: string` — trigger button label (from `nav.categories`)
- `exploreCategoriesLabel: string` — heading above the nav list (from `nav.exploreCategories`)
- `showAllTemplate: string` — "Show all {category}" template (from `nav.showAllCategory`); interpolate `{category}` client-side via `template.replace("{category}", title)`
...
A thin server component wrapper that renders `MegamenuClient` only when items are non-empty. It resolves the `nav` labels and prop-drills the three strings the client needs:
...
export async function MegamenuDesktop({ items, children }: Props) {
  if (!items.length) {
    return null;
  }

  const [categoriesLabel, exploreCategoriesLabel, showAllTemplate] = await Promise.all([
    t("nav.categories"),
    t("nav.exploreCategories"),
    t("nav.showAllCategory"),
  ]);
  ...
  return (
    <MegamenuClient
      ...
      exploreCategoriesLabel={exploreCategoriesLabel}
...
- All user-visible strings must come from the `nav` namespace via prop drilling from the server parent (`tNamespace("nav")` or `t("nav.key")` in the server wrapper) — no `useTranslations` calls in client components, no hardcoded English text.
