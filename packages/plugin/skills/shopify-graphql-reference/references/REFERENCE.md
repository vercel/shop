...
Shopify AI Toolkit validates Shopify correctness; inference is not validation. Hydrogen's types track its bundled Storefront API version while the template queries `SHOPIFY_API_VERSION` (`unstable` by default), so a field that exists only in the newer version types as `unknown` and passes typecheck until it is consumed. Hydrogen's `gql check` CLI needs a JavaScript TypeScript compiler API that TypeScript 7 does not provide, so there is no local schema gate; validate with the toolkit and exercise the operation against the store.
