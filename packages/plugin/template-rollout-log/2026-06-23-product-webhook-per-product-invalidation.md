...
- All-collections list: `getCollections` now stamps a `collection-{handle}` tag per entry (`tagCollections`), so collection **edits and deletes** bust it via the same per-collection tag the webhook fires — no broad `collections` tag needed. The create-lag gap described here (a brand-new collection's tag isn't on the already-cached list) is **closed by `all-collections-index-page`**, which adds a dedicated `collections-index` tag (on the all-collections reads only) fired on `collections/create` and `collections/delete`. See that entry for the final three-tier collections tagging (`collection-{handle}` / `collections-index` / break-glass `collections`).
