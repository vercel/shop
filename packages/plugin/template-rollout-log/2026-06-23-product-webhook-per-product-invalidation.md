...
- All-collections list: `getCollections` now stamps a `collection-{handle}` tag per entry (`tagCollections`), so collection **edits and deletes** bust it via the same per-collection tag the webhook fires — no broad `collections` tag needed. The create-lag gap described here (a brand-new collection's tag isn't on the already-cached list) is **closed by `all-collections-index-page`**, which fires the broad `collections` tag on `collections/create` and confines that tag to the all-collections reads. See that entry for the final collections tagging.
