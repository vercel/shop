...
- The Hydrogen client injects `$country`/`$language` from its creation-time `i18n` config, overriding per-request variables. The wrapper therefore creates the client with the country/language pair taken from the request's variables, preserving the template's per-call locale flow. Creation is plain config allocation, so a fresh client per call replaces any cross-request caching.
