...
- **State attributes:** `data-[state=open|closed]` → `data-open`/`data-closed`; `data-[state=checked|unchecked]` → `data-checked`/`data-unchecked`. `data-[disabled]` is unchanged (Base UI emits `data-disabled`, which the existing selector already matches).
- **Animations:** single-element popups (dropdown-menu, select, popover, tooltip, hover-card) keep `tw-animate-css` keyframes (`data-open:animate-in` / `data-closed:animate-out`), re-gated on the new attributes — kept the `@import "tw-animate-css"`. Overlays with a **separate backdrop + panel** (dialog, sheet) use Base UI's **transition model** instead (`transition` + `data-starting-style` / `data-ending-style`). Keyframes with `animation-fill-mode: none` snap back to their base state at `animationend`, and because Base UI unmounts the backdrop and panel together at the *longest* animation, the shorter one (the backdrop fade) reappeared for the gap — a visible flicker on drawer close. Transitions naturally hold their target value until unmount, eliminating the snap-back.
